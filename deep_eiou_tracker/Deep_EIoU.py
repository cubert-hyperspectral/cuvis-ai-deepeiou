...
from deep_eiou_tracker import matching
...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, feat=None, feat_history=30, det_idx=None):
...
class Deep_EIoU(object):
    ...
    def update(self, output_results, embedding):
        ...
        if len(output_results):
            ...
            # Track original indices through filtering for det_idx recovery
            orig_indices = np.arange(len(scores))
            bboxes = bboxes[lowest_inds]
            scores = scores[lowest_inds]
            orig_indices = orig_indices[lowest_inds]

            # Find high threshold detections
            remain_inds = scores > self.args.track_high_thresh
            dets = bboxes[remain_inds]
            scores_keep = scores[remain_inds]
            indices_keep = orig_indices[remain_inds]
            ...
            bboxes = []
            scores = []
            orig_indices = np.array([], dtype=int)
            dets = []
            scores_keep = []
            indices_keep = np.array([], dtype=int)
            features_keep = []

        if len(dets) > 0:
            '''Detections'''
            if self.args.with_reid:
                detections = [STrack(STrack.tlbr_to_tlwh(tlbr), s, f, det_idx=int(idx)) for
                              (tlbr, s, f, idx) in zip(dets, scores_keep, features_keep, indices_keep)]
            else:
                detections = [STrack(STrack.tlbr_to_tlwh(tlbr), s, det_idx=int(idx)) for
                              (tlbr, s, idx) in zip(dets, scores_keep, indices_keep)]
        else:
            detections = []
        ...
        if len(scores):
            ...
            indices_second = orig_indices[inds_second]
            if self.args.with_reid:
                features_second = embedding[inds_second]
        else:
            dets_second = []
            scores_second = []
            indices_second = np.array([], dtype=int)
            features_second = []
        ...
        if len(dets_second) > 0:
            '''Detections'''
            if self.args.with_reid:
                detections_second = [STrack(STrack.tlbr_to_tlwh(tlbr), s, f, det_idx=int(idx)) for
                                    (tlbr, s, f, idx) in zip(dets_second, scores_second, features_second, indices_second)]
            else:
                detections_second = [STrack(STrack.tlbr_to_tlwh(tlbr), s, det_idx=int(idx)) for
                                    (tlbr, s, idx) in zip(dets_second, scores_second, indices_second)]
        else:
            detections_second = []
